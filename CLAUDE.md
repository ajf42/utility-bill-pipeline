...
The walkthrough audience owns Measurabl's back-office function — a process-and-operations leader with high systems intuition who cannot trust black boxes. Every decision must be explicit and logged. The methodology artifacts (this file, [DECISIONS.md](DECISIONS.md), [TASKS.md](TASKS.md)) and the scale-to-production companion doc are the differentiators, not the code.
...
- [src/main.py](src/main.py) — instantiates `FastAPI(title="Utility Bill Pipeline", version="0.3.0", lifespan=_lifespan)`, mounts the bills router, exposes `GET /health` returning `{"status": "ok", "version": "0.3.0"}`. At module import calls `load_dotenv()` so a project-local `.env` (gitignored) populates `os.environ` before the lifespan runs — real OS env vars still take precedence. The lifespan handler reads `ANTHROPIC_API_KEY` and installs a `DrafterService` via `set_drafter`. Missing key is a startup-time `RuntimeError` — the prototype refuses to run partially-configured because the drafter is a load-bearing path, not optional (see ADR-013). Tests construct `TestClient(app)` without a context manager and therefore do not fire the lifespan.
